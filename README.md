...
> **Windows 安装包状态：Internal Beta / 朋友试用版。** 优先分发 `releases/Agent-Cowork-v0.2.0-internal-beta-windows-x64.zip` 与同名 `.sha256`；zip 内包含安装程序、安装程序 SHA256、beta 说明和 Windows 安装教程。它已通过本地全量门禁、安装版 smoke、依赖审计和 secrets 扫描，适合小范围可信朋友试用；但安装包仍为 `NotSigned`，未完成 OV/EV 代码签名、正式 updater endpoint、clean tag release 和外部生产发布证据，因此不应标成公开生产版。
