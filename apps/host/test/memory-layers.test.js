...
function seed() {
  ...
  fs.writeFileSync(path.join(root, '.KimiCowork', 'MEMORY.md'), '项目：Kimi Cowork', 'utf8');
...
test('loadLayeredMemory merges present layers in precedence order', () => {
  ...
  assert.ok(out.text.indexOf('用户偏好') < out.text.indexOf('项目：Kimi'));
  assert.ok(out.text.indexOf('项目：Kimi') < out.text.indexOf('本地：测试机'));
