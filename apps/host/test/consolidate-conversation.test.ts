...
function seedConversation(root: string, convId: string): void {
  appendConversationTurn(root, convId, { role: 'user', text: '我的项目代号是 Phoenix-7，我是后端负责人张伟。' });
  appendConversationTurn(root, convId, { role: 'assistant', text: '已记住：项目代号 Phoenix-7，后端负责人张伟。' });
  appendConversationTurn(root, convId, { role: 'user', text: '顺便查一下今天天气。' });
  appendConversationTurn(root, convId, { role: 'assistant', text: '今天晴。' });
}
...
test('consolidateConversation extracts topic knowledge, stores it, and clears the buffer', async () => {
  ...
  const res = await consolidateConversation({ trustedRoot: root, conversationId: 'conv-A', modelConfig, callJson });
  assert.equal(res.consolidated, true);
  assert.equal(res.stored, 3);
  assert.equal(listKnowledgeItems(root, { status: 'active' }).length, 2);
  assert.equal(listKnowledgeItems(root, { status: 'pending' }).length, 1);
  // 提炼后缓冲被清理,避免重复提炼。
  assert.equal(fs.existsSync(conversationBufferPath(root, 'conv-A')), false);
});

test('too-short conversations are skipped (no model call, no items)', async () => {
  const root = tempRoot('kcw-consol-short-');
  appendConversationTurn(root, 'conv-B', { role: 'user', text: '你好' });
  let called = false;
  const callJson = async () => { called = true; return []; };
  const res = await consolidateConversation({ trustedRoot: root, conversationId: 'conv-B', modelConfig, callJson, minTurns: 4 });
...
test('an empty extraction (nothing worth remembering) still clears the buffer', async () => {
  ...
  const res = await consolidateConversation({ trustedRoot: root, conversationId: 'conv-C', modelConfig, callJson: async () => [] });
  assert.equal(res.consolidated, true);
  assert.equal(res.stored, 0);
  assert.equal(fs.existsSync(conversationBufferPath(root, 'conv-C')), false);
});
...
test('a failing extraction keeps the buffer for a later retry and does not throw', async () => {
  ...
  const res = await consolidateConversation({ trustedRoot: root, conversationId: 'conv-D', modelConfig, callJson: async () => { throw new Error('model down'); } });
  assert.equal(res.consolidated, false);
  assert.match(String(res.reason), /fail|error|down/i);
  // 缓冲仍在,可重试
  assert.ok(readRecentTurns(root, 'conv-D').length >= 4);
});
