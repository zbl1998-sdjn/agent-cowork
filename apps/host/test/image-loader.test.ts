...
function tmp(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'kcw-img-'));
}
...
test('loadImageContentParts encodes images, skips non-images and jail escapes', () => {
  ...
  assert.ok(firstPart);
  assert.equal(firstPart.type, 'image_url');
  assert.match(firstPart.image_url.url, /^data:image\/png;base64,/);
});

test('loadImageContentParts caps the number of images', () => {
  const root = tmp();
  for (let i = 0; i < 10; i += 1) {
    fs.writeFileSync(path.join(root, `i${i}.png`), Buffer.from([0x89, 0x50]));
  }
