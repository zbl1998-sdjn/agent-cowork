...
test('verify=true: after a mutation, agent runs a read-only self-check round then finalizes', async () => {
  ...
  const out = await runAgentChat({ prompt: '改文件', modelConfig: TEST_LOCAL_MODEL_CONFIG, trustedRoot: root, tools, modelCall, approvals, verify: true, emit: (t, d) => events.push({ t, d }), runStoreRoot: path.join(root, 'runs') });
...
test('verify=true but nothing mutated: no verification round', async () => {
  ...
  const out = await runAgentChat({ prompt: '看一下', modelConfig: TEST_LOCAL_MODEL_CONFIG, trustedRoot: root, tools, modelCall, verify: true, emit: (t, d) => events.push({ t, d }), runStoreRoot: path.join(root, 'runs') });
...
test('verify=false (default): no self-check even after a mutation', async () => {
  ...
  const out = await runAgentChat({ prompt: '改文件', modelConfig: TEST_LOCAL_MODEL_CONFIG, trustedRoot: root, tools, modelCall, emit: (t, d) => events.push({ t, d }), runStoreRoot: path.join(root, 'runs') });
