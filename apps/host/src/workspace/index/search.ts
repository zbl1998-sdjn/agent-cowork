...
import type { WorkspaceChunk } from './chunk.js';
...
export type SearchWorkspaceResult = {
  ...
  sources: SearchWorkspaceSource[];
};
type WorkspaceTreeFile = { kind: string; size: number; fullPath: string };
...
function sourceFromChunk(root: string, chunk: WorkspaceChunk): SearchWorkspaceSource {
  return {
...
/** 在工作区内做关键词/RAG 检索:列文件→抽取文本→入检索器→查询,返回 { chunks, sources, indexedFiles }。 */
