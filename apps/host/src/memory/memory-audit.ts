...
export type MemoryAuditContext = { auditBus?: AuditEventBus } & Record<string, unknown>;

// 按审计文件路径缓存总线,使同一 root 复用一条流而非每次新建订阅者。
const defaultAuditBuses = new Map<string, AuditEventBus>();

/**
 * 取(或惰性创建)某 trustedRoot 对应的默认审计总线。
 */
function getDefaultAuditBus(trustedRoot: unknown): AuditEventBus {
 ...
 */
export function appendAuditEvent(trustedRoot: unknown, event: Record<string, unknown>, context: MemoryAuditContext = {}): string {
  const audit = auditPath(trustedRoot);
 ...
 */
export async function flushMemoryAuditEvents(trustedRoot: unknown): Promise<void> {
