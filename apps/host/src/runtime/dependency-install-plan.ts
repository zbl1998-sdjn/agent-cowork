// 依赖安装计划(host · L2 运行时 · runtime)
// ---------------------------------------------------------------------------
// 职责:为「按需安装外部运行时依赖」生成安装计划——校验依赖 id、供应链预检、确定安装/保留路径与子进程命令,
//       并保证更新时保留用户已装内容。供安装引导路由调用。
// 依赖:同层 dependency-plan-utils。导出:构建安装计划的函数。
import {
...
export function buildRuntimeDependencyInstallPlan(options: RuntimeDependencyInstallPlanOptions = {}) {
  ...
  const components: InstallPlanComponent[] = [];
...
export function buildRuntimeDependencyCleanupPlan(options: RuntimeDependencyCleanupPlanOptions = {}) {
  const catalog = dependencyById();
...
export function buildRuntimeDependencyUpdatePlan(options: RuntimeDependencyUpdatePlanOptions = {}) {
  const catalog = dependencyById();
  const appDataRoot = normalizeAgentCoworkRoot(options.appDataRoot);
  const selectedIds = Array.isArray(options.selectedIds) ? options.selectedIds : onDemandDependencyIds();
  const components: PlanTarget[] = [];
  ...
  ] satisfies RetainedPlanItem[]).map((item) => ({
    ...item,
