...
// 依赖:L0 path-policy + workspace/document-extractor + 同层 registry；
//       run 记录与索引写入同属 L1 storage。导出:runRecipe。
...
import { filePathFromRecipeRequest, preflightRecipeSourceFiles } from './recipe-source-files.js';
...
import type { RunRecord } from '../storage/run-store.js';
...
export async function runRecipe({
  ...
  const runContext: Record<string, unknown> = context ?? LOCAL_IDENTITY_SCOPE;
  const safeRoot = assertTrustedPath(path.resolve(trustedRoot), path.resolve(trustedRoot));
  const requestedFiles = preflightRecipeSourceFiles({ trustedRoot: safeRoot, files, owner: runContext });
  const startedAt = new Date();
  const runId = createRunId();
  const events: Record<string, unknown>[] = [];
  const scopedRunEvents = bindRunEventPublisher(runEvents, runContext);

  const emit = async (type: string, payload: Record<string, unknown> = {}): Promise<Record<string, unknown>> => {
    const enriched = scopedRunEvents
      ? await scopedRunEvents.publish(runId, { type, ...payload })
      : { seq: events.length + 1, ts: new Date().toISOString(), type, ...payload };
    events.push(enriched);
    return enriched;
  };

  await emit('user_message', { text: String(prompt || '').slice(0, 2000) });
  await emit('assistant_start', { status: 'planning', recipeId, recipeName: recipe.name });
  ...
  try {
    ...
    if (modelConfig && hasAiRecipeBuilder(recipe.id)) {
      await emit('progress', { icon: 'loader', text: `AI 正在从来源提取 ${recipe.name}…` });
      ...
      if (aiOps) await emit('progress', { icon: 'check', text: `AI 提取完成，生成 ${aiOps.length} 个产物` });
    }
    ...
    if (runsIndex) {
      try {
        runsIndex.upsert(summariseRunForIndex({ ...failRecord, runPath }, runContext), runContext);
      } catch {
  ...
  await emit('progress', { icon: 'loader', text: `正在生成 ${recipe.name} 的可审批操作…` });
  await emit('preview', { operations, count: operations.length, aiGenerated });
  await emit('awaiting_approval', { count: operations.length });
  ...
  await emit('sources', { items: sourceSummaries });

  const finishedAt = new Date();
  const durationMs = finishedAt.getTime() - startedAt.getTime();
  await emit('assistant_end', { status: 'awaiting_approval', durationMs });
  ...
  const record: RunRecord = {
    ...
    result: {
      ...
      text: `${recipe.name} 已生成 ${operations.length} 个待审批操作，尚未写入工作区。`,
    },
    events,
  };
  ...
  if (runsIndex) {
    try {
      runsIndex.upsert(summariseRunForIndex({ ...record, runPath }, runContext), runContext);
    } catch {
