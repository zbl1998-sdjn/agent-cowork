// Default model call for the agent loop: an OpenAI-compatible streaming
// chat-completions client (SSE) that accumulates content/reasoning/tool_calls
// and usage. Self-contained (only fetch + TextDecoder), so it is swappable and
...
export async function defaultAgentModelCall({ messages, tools, kimiConfig, fetchImpl = globalThis.fetch, onContent, onReasoning, signal }) {
  ...
  const headers = { authorization: `Bearer ${kimiConfig.apiKey}`, 'content-type': 'application/json', accept: 'text/event-stream' };
  if (kimiConfig.userAgent) headers['user-agent'] = kimiConfig.userAgent;
  const body = { model: kimiConfig.model, messages, tools, tool_choice: 'auto', max_tokens: kimiConfig.maxTokens || 2048, stream: true };
  if (Number.isFinite(kimiConfig.temperature)) body.temperature = kimiConfig.temperature;
  // `signal` lets the caller enforce a hard timeout (AbortController) so a hung
  // upstream connection can't stall the agent loop forever.
  ...
  for (;;) {
    ...
    while ((nl = buffer.indexOf('\n')) >= 0) {
      ...
      if (delta.reasoning_content) { reasoning += delta.reasoning_content; if (typeof onReasoning === 'function') onReasoning(delta.reasoning_content); }
      if (delta.content) { content += delta.content; if (typeof onContent === 'function') onContent(delta.content); }
