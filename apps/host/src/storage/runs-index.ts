...
export function summariseRunForIndex(runRecord: unknown, context: RunIndexContext = {}): RunIndexSummary {
  ...
  const contextHasOwner = Object.hasOwn(context, 'tenantId') || Object.hasOwn(context, 'userId');
  const recordContext = record.context;
  const owner = requireIdentityScopeFrom(
    contextHasOwner ? context : recordContext,
    { label: 'run index summary identity' },
  );
  return {
    id: record.id,
