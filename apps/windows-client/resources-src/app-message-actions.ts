...
(function () {
  function createMessageActions({
    ...
    function addClarificationCard(message: any, prompt: any, options: any) {
      if (!message?.body) {
        return;
      }
      ...
      for (const option of options) {
        ...
        button.addEventListener("click", () => {
          choices.querySelectorAll("button").forEach((node: any) => {
            node.disabled = true;
          });
    ...
    function addApprovalActions(message: any) {
      if (!message?.body) {
        return;
      }
