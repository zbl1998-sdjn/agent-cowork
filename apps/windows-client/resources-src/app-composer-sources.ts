...
(function () {
  function createComposerSources({
    state,
    searchLocalFiles,
    getJson,
    compactText,
    ...
    function templateItems(query: any) {
      const q = String(query || "").toLowerCase();
      return state.recipes
        .filter((recipe: any) => !q || `${recipe.name} ${recipe.id} ${recipe.summary || ""}`.toLowerCase().includes(q))
        .slice(0, 6)
        .map((recipe: any) => ({ kind: "template", id: recipe.id, title: recipe.name, detail: recipe.summary || recipe.id, recipe }));
    }

    async function mentionItems(query: any) {
      const results = await searchLocalFiles(query);
      return results.slice(0, 8).map((file: any) => ({
        kind: "mention",
        title: file.path,
    ...
    async function historyRunItems(query: any) {
      if (!state.hostApi) {
        return [];
      }
