...
(function () {
  function createRecipeRunner({
    ...
    async function runRecipePlan(prompt: any, recipe: any) {
      ...
      const result = await postJson(`/api/recipes/${encodeURIComponent(recipe.id)}/run`, {
        ...
        prompt,
        files: files.map((file: any) => file.fullPath),
        maxSize: 2 * 1024 * 1024,
      ...
      const sourceCopy = state.lastSources.length > 0
        ? state.lastSources.map((source: any) => source.relativePath || basename(source.path)).join("、")
        : "无来源文件";
      const sourceExcerpt = compactText(state.lastSources.find((source: any) => source.excerpt)?.excerpt || "", 150);
