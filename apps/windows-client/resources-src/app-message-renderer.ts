...
(function () {
  function createMessageRenderer({
    ...
    function showChatResponse(message: any) {
    ...
    function setMessageStatus(message: any, status: any) {
      if (!message?.statusEl) {
        return;
      }
    ...
    function appendMessage(role: any, text: any, { status = "", meta = "" }: any = {}) {
    ...
    function appendUserMessage(text: any) {
      return appendMessage("user", text);
    }

    function appendAssistantMessage(text: any, options: any = {}) {
      return appendMessage("assistant", text, options);
    }

    function appendMessageText(message: any, text: any) {
    ...
    function addProgressLines(message: any, items: any) {
    ...
    function addPreviewCard(message: any, operations: any, summary: any = "等待审批") {
      if (!message?.body) {
        return;
      }
      const card = document.createElement("div");
    ...
    function markApprovalDone(message: any) {
    ...
    function addArtifactCard(message: any, title: any, description: any, pathText: any) {
      if (!message?.body) {
        return;
      }
      const card = document.createElement("div");
    ...
    function addSourcesFooter(message: any, sources: any) {
    ...
    function clearConversation() {
      conversationTimeline?.querySelectorAll(".message-bubble").forEach((node: any) => node.remove());
