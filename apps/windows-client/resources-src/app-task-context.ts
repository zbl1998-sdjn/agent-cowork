...
(function () {
  function createTaskContext({ state, postJson, selectedRecipe, renderRecipes }: any) {
    function textCandidate(files: any) {
      return files.find((file: any) => /\.(md|txt|csv|docx|xlsx|pptx|pdf)$/i.test(file.path)) || files.find((file: any) => file.kind === "file");
    }
    ...
    function recipeFiles() {
      ...
      return activeFiles()
        ...
        .filter((file: any) => /\.(md|txt|csv|docx|xlsx|pptx|pdf|json|log)$/i.test(file.path))
        .filter((file: any) => {
    ...
    function inferRecipeId(prompt: any) {
      ...
      return pairs.find(([, pattern]: any) => pattern.test(text))?.[0] || "";
    }

    function maybeSelectRecipe(prompt: any) {
    ...
    async function readCandidateSummary(candidate: any) {
      if (!candidate) {
