...
(function () {
  function createFileUploadController({
    ...
    async function uploadSelectedFiles(fileList: FileList | File[] | null | undefined, sourceLabel: any) {
      ...
      const totalBytes = selected.reduce((sum: any, file: any) => sum + file.size, 0);
      ...
      state.uploadedFiles = imported.imported.map((file: any) => ({
