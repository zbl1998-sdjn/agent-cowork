...
// ---------------------------------------------------------------------------
// 职责:把 artifact、工作台渲染、任务上下文、Agent 调用和消息渲染这些横切能力
//   合成为一个 services 对象。app.ts 仍保持真正的 composition root,本文件只负责
//   构造可复用服务,不绑定 DOM 事件、不发起启动流程。
(function () {
  function createShellServices({ state, elements, placeholders, utils, api }: any) {
    ...
    // agentEngineRunner 封装直接模型调用,这里只注入 UI 状态更新函数,避免反向依赖渲染模块。
    const agentEngineRunner = window.AgentCoworkAgentEngineRunner.createAgentEngineRunner({
      state,
      postJson: api.postJson,
