...
(function () {
  function createWorkbenchRenderer({
    ...
    function setRunChip(text: any, variant: any = "muted") {
      if (!runChip) {
        return;
      }
    ...
    function renderInteraction(items: any, subtitle: any = "任务运行中") {
    ...
    function setWorkbenchCopy(view: any) {
      if (view === "code") {
    ...
    function setView(view: any) {
      ...
      document.querySelectorAll<HTMLElement>(".mode-tab").forEach((tab: any) => {
      ...
      document.querySelectorAll<HTMLElement>(".view-panel").forEach((panel: any) => {
    ...
    function summarizeFiles(files: any) {
      const docs = files.filter((file: any) => file.kind === "file").length;
      const dirs = files.filter((file: any) => file.kind === "directory").length;
      workspaceMeta.textContent = `已发现 ${docs} 个文件、${dirs} 个目录，可读取文本并生成审批操作。`;
    }

    function renderFiles(files: any) {
      fileList.replaceChildren();
      const visible = files.filter((file: any) => file.kind === "file").slice(0, 4);
    ...
    function renderOperations(operations: any) {
    ...
    function selectedRecipe() {
      return state.recipes.find((recipe: any) => recipe.id === state.selectedRecipeId) || null;
    }

    function renderRecipes(recipes: any) {
