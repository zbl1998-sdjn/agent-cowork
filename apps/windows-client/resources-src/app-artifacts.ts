...
(function () {
  ...
  function createArtifactCatalog({ state, artifactText, artifactPath, artifactList, getJson, basename }: any) {
    function setArtifact(message: any, pathText: any = artifactPath?.textContent || "") {
      if (artifactText) {
    ...
    function openArtifactView(item: any) {
    ...
    function renderArtifactCatalog(items: any) {
      if (!artifactList) {
        return;
      }
