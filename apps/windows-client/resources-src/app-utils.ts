(function () {
  function compactText(text: any, maxLength: any = 220) {
  ...
  function joinWin(root: any, ...parts: any[]) {
    return [root.replace(/[\\/]+$/, ""), ...parts.map((part: any) => String(part).replace(/^[\\/]+|[\\/]+$/g, ""))].join("\\");
  }

  function uniqueStamp(date: any = new Date()) {
    const timestamp = date.toISOString().replace(/[-:.TZ]/g, "").slice(0, 17);
    const suffix = Math.random().toString(16).slice(2, 6);
    return `${timestamp}-${suffix}`;
  }

  function idempotencyKey(prefix: any = "kcw") {
  ...
  function messageStatusClass(status: any) {
  ...
  function progressStateFromIcon(icon: any) {
    if (icon === "check") return "done";
    if (icon === "loader") return "running";
    return "wait";
  }

  function shortRunId(runId: any) {
    return String(runId || "").split("_").slice(-1)[0] || runId;
  }

  function runStatusText(status: any) {
    if (status === "succeeded") return "完成";
    if (status === "failed") return "失败";
    return "运行中";
  }

  function runTypeText(run: any) {
