...
(function () {
  function createKimiRunner({ state, postJson, setRunChip, setStatus, shortRunId }: any) {
    async function tryKimiApiPlan(prompt: any, summary: any) {
    ...
    async function tryKimiChat(prompt: any, summary: any) {
      if (!state.kimiApiEnabled) {
        setRunChip("Kimi API 未配置", "muted");
        return {
          used: false,
