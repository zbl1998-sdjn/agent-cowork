...
static const wchar_t *ACW_CLASS_NAME = L"AgentCoworkWindow";

#define ACW_MAX_FILES 240
#define ACW_CONTEXT_MAX_FILES 3
#define ACW_CONTEXT_SNIPPET_BYTES 768
#define ACW_TEMPLATE_COUNT 8
#define ACW_NAV_COUNT 9

#define ACW_ID_NEW_CHAT 1001
#define ACW_ID_BROWSE 1002
#define ACW_ID_RUN 1003
#define ACW_ID_APPROVE 1004
#define ACW_ID_DEVELOPER 1005
#define ACW_ID_BROWSER 1006
#define ACW_ID_PROMPT 2001
#define ACW_ID_FILE_LIST 2002
#define ACW_ID_ARTIFACT 2003
#define ACW_ID_TEMPLATE_BASE 3000

#define ACW_COLOR_BG RGB(247, 248, 250)
#define ACW_COLOR_SURFACE RGB(255, 255, 255)
#define ACW_COLOR_SIDEBAR RGB(248, 249, 251)
#define ACW_COLOR_PANEL RGB(252, 252, 253)
#define ACW_COLOR_BORDER RGB(226, 228, 232)
#define ACW_COLOR_BORDER_DARK RGB(206, 210, 216)
#define ACW_COLOR_TEXT RGB(20, 22, 26)
#define ACW_COLOR_MUTED RGB(108, 112, 120)
#define ACW_COLOR_FAINT RGB(147, 151, 158)
#define ACW_COLOR_SOFT RGB(243, 244, 246)
#define ACW_COLOR_SOFT_ACTIVE RGB(235, 237, 241)
#define ACW_COLOR_ACCENT RGB(255, 76, 64)
#define ACW_COLOR_ACCENT_SOFT RGB(255, 241, 239)
#define ACW_COLOR_BLACK RGB(14, 15, 17)

typedef struct AcwAppState {
    HFONT font_ui;
    HFONT font_ui_bold;
    HFONT font_small;
    HFONT font_micro;
    ...
    HWND browser_button;
    HWND template_buttons[ACW_TEMPLATE_COUNT];
    ...
    wchar_t file_paths[ACW_MAX_FILES][MAX_PATH];
...
static AcwAppState g_app;
static wchar_t g_initial_workspace[MAX_PATH];

static const wchar_t *ACW_TEMPLATES[ACW_TEMPLATE_COUNT] = {
    L"文件夹整理",
    L"会议纪要",
    L"合同摘要",
    L"发票归档",
...
static const wchar_t *ACW_NAV_ITEMS[ACW_NAV_COUNT] = {
    L"PPT",
    L"文档",
    L"深度研究",
    L"网站",
...
static const wchar_t *ACW_NAV_ICONS[ACW_NAV_COUNT] = {
    L"▱",
    L"□",
    L"⌕",
    L"▭",
...
static int acw_min_int(int a, int b) {
    return a < b ? a : b;
}

static int acw_max_int(int a, int b) {
    return a > b ? a : b;
}

static void acw_set_status(const wchar_t *status) {
    wcsncpy_s(g_app.status_line, sizeof(g_app.status_line) / sizeof(g_app.status_line[0]), status, _TRUNCATE);
}

static const wchar_t *acw_skip_spaces_local(const wchar_t *value) {
...
static void acw_trim_trailing_spaces_local(wchar_t *value) {
...
static bool acw_parse_workspace_from_text(const wchar_t *text, wchar_t *workspace, size_t workspace_len) {
    if (!text || text[0] == L'\0') {
        return false;
    }
    ...
    if (value) {
        ...
        value = acw_skip_spaces_local(value);
    }
    ...
    acw_trim_trailing_spaces_local(workspace);
    return workspace[0] != L'\0';
}

static void acw_detect_initial_workspace(void) {
    ...
    if (acw_parse_workspace_from_text(GetCommandLineW(), parsed_workspace, sizeof(parsed_workspace) / sizeof(parsed_workspace[0]))) {
...
static HFONT acw_create_named_font(int size, int weight, const wchar_t *family) {
    return CreateFontW(
        -size,
        0,
        0,
...
static HFONT acw_create_font(int size, int weight) {
    return acw_create_named_font(size, weight, L"Microsoft YaHei UI");
}

static void acw_draw_line(HDC dc, int x1, int y1, int x2, int y2, COLORREF color) {
...
static void acw_draw_round_rect(HDC dc, RECT rect, COLORREF fill, COLORREF border, int radius) {
...
static void acw_draw_soft_shadow(HDC dc, RECT rect, int radius) {
    RECT layer = rect;
    OffsetRect(&layer, 0, 8);
    InflateRect(&layer, 8, 8);
    acw_draw_round_rect(dc, layer, RGB(243, 244, 246), RGB(243, 244, 246), radius + 10);

    layer = rect;
    OffsetRect(&layer, 0, 4);
    InflateRect(&layer, 4, 4);
    acw_draw_round_rect(dc, layer, RGB(237, 239, 243), RGB(237, 239, 243), radius + 6);
}

static void acw_draw_text(HDC dc, HFONT font, const wchar_t *text, RECT rect, COLORREF color, UINT format) {
...
static void acw_draw_badge(HDC dc, RECT rect, const wchar_t *text, COLORREF fill, COLORREF border, COLORREF color) {
    acw_draw_round_rect(dc, rect, fill, border, 12);
    acw_draw_text(dc, g_app.font_micro, text, rect, color, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
}

static HWND acw_create_owner_button(HWND parent, int id, const wchar_t *text) {
...
static bool acw_is_template_id(int id) {
    return id >= ACW_ID_TEMPLATE_BASE && id < ACW_ID_TEMPLATE_BASE + ACW_TEMPLATE_COUNT;
}

static void acw_draw_button(const DRAWITEMSTRUCT *item) {
    wchar_t text[128] = L"";
    ...
    bool template_selected = acw_is_template_id(id) && (id - ACW_ID_TEMPLATE_BASE == g_app.selected_template);

    COLORREF fill = ACW_COLOR_SURFACE;
    COLORREF border = ACW_COLOR_BORDER;
    COLORREF text_color = ACW_COLOR_TEXT;
    HFONT font = g_app.font_ui;
    UINT text_format = DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS;
    int radius = 16;

    if (id == ACW_ID_RUN) {
        fill = pressed ? RGB(48, 49, 52) : ACW_COLOR_BLACK;
        border = fill;
        text_color = RGB(255, 255, 255);
        font = g_app.font_ui_bold;
    } else if (id == ACW_ID_APPROVE) {
        fill = pressed ? RGB(225, 56, 45) : ACW_COLOR_ACCENT;
        border = fill;
        text_color = RGB(255, 255, 255);
        font = g_app.font_ui_bold;
    } else if (id == ACW_ID_BROWSE || id == ACW_ID_NEW_CHAT) {
        fill = pressed ? ACW_COLOR_SOFT_ACTIVE : ACW_COLOR_SURFACE;
        text_format = DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS;
        font = g_app.font_ui_bold;
    } else if (template_selected) {
        fill = ACW_COLOR_BLACK;
        border = ACW_COLOR_BLACK;
        text_color = RGB(255, 255, 255);
        font = g_app.font_small;
        radius = 18;
    } else if (id == ACW_ID_DEVELOPER) {
        fill = ACW_COLOR_ACCENT_SOFT;
        border = RGB(255, 216, 211);
        text_format = DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS;
        font = g_app.font_ui_bold;
    } else if (acw_is_template_id(id)) {
        fill = ACW_COLOR_SURFACE;
        border = RGB(221, 224, 229);
        font = g_app.font_small;
        radius = 18;
    }

    if (focused && id != ACW_ID_RUN && id != ACW_ID_APPROVE && !template_selected) {
        border = RGB(158, 166, 178);
    }

    RECT rect = item->rcItem;
    InflateRect(&rect, -1, -1);
    acw_draw_round_rect(item->hDC, rect, fill, border, radius);

    RECT text_rect = rect;
    if (text_format & DT_LEFT) {
        text_rect.left += 18;
        text_rect.right -= 18;
        if (id == ACW_ID_NEW_CHAT) {
            text_rect.right -= 76;
        }
    }
    acw_draw_text(item->hDC, font, text, text_rect, text_color, text_format);

    if (id == ACW_ID_NEW_CHAT) {
        RECT key = {rect.right - 76, rect.top + 8, rect.right - 16, rect.bottom - 8};
        acw_draw_round_rect(item->hDC, key, ACW_COLOR_SOFT, ACW_COLOR_SOFT, 8);
        acw_draw_text(item->hDC, g_app.font_micro, L"Ctrl  K", key, ACW_COLOR_MUTED, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    }
}

static bool acw_has_document_extension(const wchar_t *name) {
...
static bool acw_has_text_summary_extension(const wchar_t *name) {
    const wchar_t *dot = wcsrchr(name, L'.');
    if (!dot) {
        return false;
    }
...
static bool acw_skip_directory(const wchar_t *name) {
...
static void acw_append_text(wchar_t *out, size_t out_len, const wchar_t *text) {
...
static void acw_append_format(wchar_t *out, size_t out_len, const wchar_t *format, ...) {
    wchar_t line[1200];
    line[0] = L'\0';
    va_list args;
    ...
    va_end(args);
    acw_append_text(out, out_len, line);
}

static void acw_join_path(wchar_t *out, size_t out_len, const wchar_t *base, const wchar_t *child) {
...
static void acw_normalize_snippet(wchar_t *value) {
    ...
    *write = L'\0';
    acw_trim_trailing_spaces_local(value);
}

static bool acw_read_text_snippet(const wchar_t *relative, wchar_t *snippet, size_t snippet_len, DWORD *size_out) {
    ...
    acw_join_path(full_path, sizeof(full_path) / sizeof(full_path[0]), g_app.trusted_root, relative);
    ...
    char bytes[ACW_CONTEXT_SNIPPET_BYTES + 1];
    DWORD bytes_read = 0;
    BOOL ok = ReadFile(file, bytes, ACW_CONTEXT_SNIPPET_BYTES, &bytes_read, NULL);
    CloseHandle(file);
    if (!ok || bytes_read == 0) {
        return false;
    }
    ...
    acw_normalize_snippet(snippet);
    return snippet[0] != L'\0';
}

static void acw_build_context_summary(wchar_t *out, size_t out_len, int *read_count_out, int *skipped_count_out) {
    if (!out || out_len == 0) {
        return;
    }
    out[0] = L'\0';
    ...
    acw_append_text(out, out_len, L"本地内容摘要\r\n");
    if (g_app.trusted_root[0] == L'\0' || g_app.file_count <= 0) {
        acw_append_text(out, out_len, L"- 尚未选择信任工作区或未扫描到可处理文件。\r\n\r\n");
        if (read_count_out) {
            *read_count_out = 0;
        }
    ...
    for (int i = 0; i < g_app.file_count; i++) {
        ...
        if (!acw_has_text_summary_extension(relative)) {
            skipped_count++;
            continue;
        }
        ...
        wchar_t snippet[512];
        DWORD file_size = 0;
        if (!acw_read_text_snippet(relative, snippet, sizeof(snippet) / sizeof(snippet[0]), &file_size)) {
            skipped_count++;
            continue;
        }

        acw_append_format(out, out_len, L"- %s (%lu bytes)：%s\r\n", relative, (unsigned long)file_size, snippet);
        read_count++;
        if (read_count >= ACW_CONTEXT_MAX_FILES) {
    ...
    if (read_count == 0) {
        acw_append_text(out, out_len, L"- 已扫描文件，但当前只预览 TXT / Markdown / CSV；PDF / Office 文件会留给后续解析器处理。\r\n");
    }
    if (skipped_count > 0) {
        acw_append_format(out, out_len, L"- 跳过：%d 个二进制、Office/PDF、不可读或超出本轮摘要上限的文件。\r\n", skipped_count);
    }
    acw_append_text(out, out_len, L"\r\n");
...
static const wchar_t *acw_file_name_from_relative(const wchar_t *relative) {
...
static void acw_clear_pending_move(void) {
...
static bool acw_build_pending_move_preview(void) {
    acw_clear_pending_move();
    if (g_app.trusted_root[0] == L'\0' || g_app.file_count <= 0) {
        return false;
    }

    const wchar_t *source_relative = g_app.file_paths[0];
    const wchar_t *file_name = acw_file_name_from_relative(source_relative);
    ...
    swprintf_s(
        ...
        ACW_TEMPLATES[g_app.selected_template],
        file_name);
    acw_join_path(g_app.pending_move_from, sizeof(g_app.pending_move_from) / sizeof(g_app.pending_move_from[0]), g_app.trusted_root, source_relative);
    acw_join_path(g_app.pending_move_to, sizeof(g_app.pending_move_to) / sizeof(g_app.pending_move_to[0]), g_app.trusted_root, g_app.pending_move_to_relative);
    wcsncpy_s(
...
static void acw_scan_files_recursive(const wchar_t *root, const wchar_t *relative, int depth) {
    if (depth > 4 || g_app.file_count >= ACW_MAX_FILES) {
        return;
    }

    wchar_t base[MAX_PATH];
    wchar_t search[MAX_PATH];
    acw_join_path(base, sizeof(base) / sizeof(base[0]), root, relative);
    ...
    do {
        ...
        if ((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0) {
            if (!acw_skip_directory(data.cFileName)) {
                acw_scan_files_recursive(root, child_relative, depth + 1);
            }
            continue;
        }

        if (acw_has_document_extension(data.cFileName)) {
            wcsncpy_s(
            ...
            g_app.file_count++;
            if (g_app.file_count >= ACW_MAX_FILES) {
                break;
            }
        }
...
static void acw_scan_trusted_root(void) {
    ...
    acw_scan_files_recursive(g_app.trusted_root, L"", 0);

    wchar_t status[256];
    swprintf_s(status, sizeof(status) / sizeof(status[0]), L"已信任工作区，扫描到 %d 个可处理文件。", g_app.file_count);
    acw_set_status(status);
}

static bool acw_ensure_directory(const wchar_t *path) {
...
static bool acw_prepare_app_directories(wchar_t *artifacts_dir, size_t artifacts_len, wchar_t *audit_dir, size_t audit_len, wchar_t *rollback_dir, size_t rollback_len) {
    if (g_app.trusted_root[0] == L'\0') {
        return false;
    }
    ...
    return acw_ensure_directory(app_dir) && acw_ensure_directory(artifacts_dir) && acw_ensure_directory(audit_dir) && acw_ensure_directory(rollback_dir);
}

static void acw_make_batch_id(wchar_t *batch_id, size_t batch_len) {
    SYSTEMTIME now;
    GetLocalTime(&now);
    swprintf_s(
        batch_id,
...
static void acw_json_write_string(FILE *file, const wchar_t *value) {
    fputwc(L'"', file);
...
static bool acw_write_approved_artifact(const wchar_t *plan_text, wchar_t *error, size_t error_len) {
    wchar_t artifacts_dir[MAX_PATH];
    wchar_t audit_dir[MAX_PATH];
    wchar_t rollback_dir[MAX_PATH];
    if (!acw_prepare_app_directories(artifacts_dir, sizeof(artifacts_dir) / sizeof(artifacts_dir[0]), audit_dir, sizeof(audit_dir) / sizeof(audit_dir[0]), rollback_dir, sizeof(rollback_dir) / sizeof(rollback_dir[0]))) {
        wcsncpy_s(error, error_len, L"无法创建 .AgentCowork 本地产物目录。", _TRUNCATE);
        return false;
    }

    wchar_t batch_id[64];
    acw_make_batch_id(batch_id, sizeof(batch_id) / sizeof(batch_id[0]));
    ...
    acw_json_write_string(audit, batch_id);
    fwprintf(audit, L",\"template\":");
    acw_json_write_string(audit, ACW_TEMPLATES[g_app.selected_template]);
    fwprintf(audit, L",\"trusted_root\":");
    acw_json_write_string(audit, g_app.trusted_root);
    fwprintf(audit, L",\"artifact_path\":");
    acw_json_write_string(audit, artifact_path);
    ...
    acw_json_write_string(rollback, batch_id);
    fwprintf(rollback, L",\"operation\":\"write_new_artifact\",\"artifact_path\":");
    acw_json_write_string(rollback, artifact_path);
...
static bool acw_apply_pending_move(wchar_t *error, size_t error_len) {
    ...
    swprintf_s(move_template_dir, sizeof(move_template_dir) / sizeof(move_template_dir[0]), L"%s\\%s", move_root, ACW_TEMPLATES[g_app.selected_template]);
    if (!acw_ensure_directory(move_root) || !acw_ensure_directory(move_template_dir)) {
    ...
    acw_json_write_string(audit, g_app.pending_move_from);
    fwprintf(audit, L",\"to\":");
    acw_json_write_string(audit, g_app.pending_move_to);
    fwprintf(audit, L",\"status\":\"done\"}\n");
    fclose(audit);

    fwprintf(rollback, L"{\"operation\":\"move_file\",\"from\":");
    acw_json_write_string(rollback, g_app.pending_move_to);
    fwprintf(rollback, L",\"to\":");
    acw_json_write_string(rollback, g_app.pending_move_from);
...
static void acw_show_workspace_ready(void) {
    acw_scan_trusted_root();
...
static void acw_select_workspace(HWND window) {
...
static void acw_generate_plan(HWND window) {
    ...
    const wchar_t *template_name = ACW_TEMPLATES[g_app.selected_template];

    wchar_t context_summary[2500];
    int context_read_count = 0;
    int context_skipped_count = 0;
    acw_build_context_summary(
        context_summary,
    ...
    if (acw_build_pending_move_preview()) {
        swprintf_s(
            move_preview,
    ...
    acw_set_status(L"计划已生成，等待审批中心确认。");
    InvalidateRect(window, NULL, TRUE);
}

static void acw_approve_plan(HWND window) {
    if (g_app.trusted_root[0] == L'\0') {
        SetWindowTextW(g_app.artifact_edit, L"请先选择本地信任工作区。审批执行只会写入该工作区内的 .AgentCowork 安全产物目录。");
        acw_set_status(L"审批被阻止：尚未选择信任工作区。");
        InvalidateRect(window, NULL, TRUE);
        return;
    }
    ...
    if (wcsstr(current, L"Agent Cowork 执行计划") == NULL) {
        acw_generate_plan(window);
        GetWindowTextW(g_app.artifact_edit, current, (int)(sizeof(current) / sizeof(current[0])));
    }

    wchar_t error[256] = L"";
    bool wrote_artifact = acw_write_approved_artifact(current, error, sizeof(error) / sizeof(error[0]));
    ...
    if (wrote_artifact && had_move_preview) {
        move_applied = acw_apply_pending_move(move_error, sizeof(move_error) / sizeof(move_error[0]));
    }
    ...
    if (wrote_artifact) {
        if (had_move_preview && move_applied) {
            ...
            acw_scan_trusted_root();
            acw_set_status(L"审批已执行：产物、文件移动、审计日志和回滚日志已写入本地工作区。");
            ...
            swprintf_s(
                ...
                g_app.last_audit_path,
                g_app.last_rollback_path);
            acw_set_status(L"审批部分执行：产物已写入，文件移动被阻止。");
        } else {
            swprintf_s(
                approved,
                sizeof(approved) / sizeof(approved[0]),
                ...
                g_app.last_audit_path,
                g_app.last_rollback_path);
            acw_set_status(L"审批已执行：产物、审计日志和回滚日志已写入本地工作区。");
        }
    } else {
        swprintf_s(
            approved,
        ...
        acw_set_status(L"审批执行失败，未改动用户文件。");
    }
    SetWindowTextW(g_app.artifact_edit, approved);
    InvalidateRect(window, NULL, TRUE);
}

static void acw_create_controls(HWND window) {
    g_app.new_chat_button = acw_create_owner_button(window, ACW_ID_NEW_CHAT, L"+  新建会话");
    g_app.browse_button = acw_create_owner_button(window, ACW_ID_BROWSE, L"+  选择本地文件夹");
    g_app.run_button = acw_create_owner_button(window, ACW_ID_RUN, L"生成计划");
    g_app.approve_button = acw_create_owner_button(window, ACW_ID_APPROVE, L"审批执行");
    g_app.developer_button = acw_create_owner_button(window, ACW_ID_DEVELOPER, L"Developer Mode");
    g_app.browser_button = acw_create_owner_button(window, ACW_ID_BROWSER, L"浏览器视图");

    for (int i = 0; i < ACW_TEMPLATE_COUNT; i++) {
        g_app.template_buttons[i] = acw_create_owner_button(window, ACW_ID_TEMPLATE_BASE + i, ACW_TEMPLATES[i]);
    }
    ...
    g_app.file_list = CreateWindowExW(
        ...
        1,
        window,
        (HMENU)(INT_PTR)ACW_ID_FILE_LIST,
        GetModuleHandleW(NULL),
        NULL);
...
static void acw_layout_controls(HWND window) {
    RECT client;
    GetClientRect(window, &client);
    int width = client.right - client.left;
    int height = client.bottom - client.top;

    int sidebar = acw_min_int(304, acw_max_int(272, width / 5));
    int content_x = sidebar + 8;
    int content_w = acw_max_int(560, width - content_x - 8);
    int card_x = content_x + 24;
    int card_w = content_w - 48;
    bool compact = height < 780;
    int prompt_w = acw_min_int(960, card_w - 96);
    int prompt_x = content_x + (content_w - prompt_w) / 2;
    int brand_y = compact ? 92 : acw_max_int(118, height / 4);
    ...
    int panel_h = acw_max_int(172, height - bottom_y - 42);
    int left_panel_w = acw_min_int(392, acw_max_int(320, (card_w - 18) / 3));
    ...
    if (g_app.webview_visible) {
        ...
        for (int i = 0; i < ACW_TEMPLATE_COUNT; i++) ShowWindow(g_app.template_buttons[i], SW_HIDE);
        ...
        for (int i = 0; i < ACW_TEMPLATE_COUNT; i++) ShowWindow(g_app.template_buttons[i], SW_SHOW);
...
static void acw_draw_sidebar(HDC dc, RECT client, int sidebar) {
    ...
    acw_draw_round_rect(dc, logo_rect, ACW_COLOR_BLACK, ACW_COLOR_BLACK, 10);
    acw_draw_text(dc, g_app.font_ui_bold, L"K", logo_rect, RGB(255, 255, 255), DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    RECT dot = {45, 30, 53, 38};
    acw_draw_round_rect(dc, dot, RGB(48, 126, 255), RGB(48, 126, 255), 8);

    RECT collapse = {sidebar - 48, 28, sidebar - 18, 56};
    acw_draw_text(dc, g_app.font_ui, L"▱", collapse, ACW_COLOR_MUTED, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    RECT tabs = {14, 76, sidebar - 14, 110};
    acw_draw_round_rect(dc, tabs, RGB(242, 243, 245), RGB(242, 243, 245), 14);
    int tab_w = (tabs.right - tabs.left) / 3;
    RECT active_tab = {tabs.left + tab_w, tabs.top + 2, tabs.left + tab_w * 2, tabs.bottom - 2};
    acw_draw_round_rect(dc, active_tab, ACW_COLOR_SURFACE, ACW_COLOR_BORDER, 12);
    ...
    acw_draw_text(dc, g_app.font_small, L"Chat", tab_chat, ACW_COLOR_MUTED, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    acw_draw_text(dc, g_app.font_small, L"Cowork", tab_cowork, ACW_COLOR_TEXT, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    acw_draw_text(dc, g_app.font_small, L"Code", tab_code, ACW_COLOR_MUTED, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    int y = compact ? 176 : 190;
    for (int i = 0; i < ACW_NAV_COUNT; i++) {
        RECT icon = {22, y + 6, 42, y + 28};
        RECT row = {54, y, sidebar - 22, y + 34};
        COLORREF color = i >= 5 ? ACW_COLOR_TEXT : ACW_COLOR_MUTED;
        HFONT font = i >= 5 ? g_app.font_ui_bold : g_app.font_ui;

        if (i == ACW_NAV_COUNT - 1) {
            RECT selected = {14, y - 5, sidebar - 14, y + 39};
            acw_draw_round_rect(dc, selected, ACW_COLOR_SURFACE, ACW_COLOR_BORDER, 15);
        }

        if (i == ACW_NAV_COUNT - 1) {
            acw_draw_round_rect(dc, icon, ACW_COLOR_ACCENT_SOFT, RGB(255, 225, 220), 10);
            acw_draw_text(dc, g_app.font_micro, ACW_NAV_ICONS[i], icon, ACW_COLOR_ACCENT, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        } else {
            acw_draw_text(dc, g_app.font_small, ACW_NAV_ICONS[i], icon, color, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        }
        acw_draw_text(dc, font, ACW_NAV_ITEMS[i], row, color, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
    ...
    acw_draw_text(dc, g_app.font_small, L"历史会话", recent_title, ACW_COLOR_TEXT, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
    ...
    acw_draw_text(dc, g_app.font_small, L"Agent 岗位要点", recent1, ACW_COLOR_MUTED, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
    acw_draw_text(dc, g_app.font_small, L"合同摘要与归档", recent2, ACW_COLOR_MUTED, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);

    RECT workspace_title = {22, client.bottom - 174, sidebar - 22, client.bottom - 150};
    acw_draw_text(dc, g_app.font_small, L"当前工作区", workspace_title, ACW_COLOR_TEXT, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
    ...
    acw_draw_text(dc, g_app.font_small, root, root_rect, ACW_COLOR_MUTED, DT_LEFT | DT_TOP | DT_WORDBREAK | DT_END_ELLIPSIS);

    RECT user = {22, client.bottom - 44, sidebar - 22, client.bottom - 18};
    acw_draw_text(dc, g_app.font_small, L"~  Allegretto", user, ACW_COLOR_TEXT, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
}

static void acw_draw_main(HDC dc, RECT client, int sidebar) {
    int content_x = sidebar + 8;
    int content_w = client.right - content_x - 8;
    RECT main_card = {content_x, 48, client.right - 8, client.bottom - 10};
    acw_draw_round_rect(dc, main_card, ACW_COLOR_SURFACE, ACW_COLOR_BORDER, 14);
    ...
    int prompt_w = acw_min_int(960, card_w - 96);
    int prompt_x = content_x + (content_w - prompt_w) / 2;
    int brand_y = compact ? 92 : acw_max_int(118, client.bottom / 4);
    int prompt_y = brand_y + (compact ? 96 : 130);
    int prompt_h = compact ? 132 : 148;
    int template_y = prompt_y + prompt_h + (compact ? 16 : 22);
    int bottom_y = template_y + (compact ? 76 : 86);
    int panel_h = acw_max_int(172, client.bottom - bottom_y - 42);
    int left_panel_w = acw_min_int(392, acw_max_int(320, (card_w - 18) / 3));
    int file_panel_x = card_x;
    int artifact_x = file_panel_x + left_panel_w + 18;
    int artifact_w = card_w - left_panel_w - 18;
    ...
    acw_draw_badge(dc, local_badge, L"Local First", RGB(244, 246, 249), RGB(234, 236, 240), ACW_COLOR_MUTED);
    acw_draw_badge(dc, policy_badge, L"审批后执行", ACW_COLOR_ACCENT_SOFT, RGB(255, 224, 220), RGB(194, 64, 55));
    acw_draw_badge(dc, model_badge, L"Kimi API 默认", RGB(244, 246, 249), RGB(234, 236, 240), ACW_COLOR_MUTED);

    RECT brand = {content_x, brand_y, client.right - 8, brand_y + 76};
    acw_draw_text(dc, g_app.font_brand, L"AGENT", brand, RGB(0, 0, 0), DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    RECT sub = {content_x, brand_y + 78, client.right - 8, brand_y + 106};
    acw_draw_text(dc, g_app.font_small, L"Cowork · 本地文件工作台 · Office Mode", sub, ACW_COLOR_MUTED, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    RECT prompt_card = {prompt_x, prompt_y, prompt_x + prompt_w, prompt_y + prompt_h};
    acw_draw_soft_shadow(dc, prompt_card, 30);
    acw_draw_round_rect(dc, prompt_card, ACW_COLOR_SURFACE, RGB(204, 209, 216), 30);
    RECT prompt_inner_line = {prompt_card.left + 18, prompt_card.bottom - 62, prompt_card.right - 18, prompt_card.bottom - 61};
    acw_draw_line(dc, prompt_inner_line.left, prompt_inner_line.top, prompt_inner_line.right, prompt_inner_line.top, RGB(241, 242, 244));

    RECT plus = {prompt_x + 20, prompt_y + prompt_h - 54, prompt_x + 58, prompt_y + prompt_h - 16};
    acw_draw_round_rect(dc, plus, ACW_COLOR_SURFACE, ACW_COLOR_BORDER, 18);
    acw_draw_text(dc, g_app.font_title, L"+", plus, ACW_COLOR_TEXT, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    RECT agent = {prompt_x + 70, prompt_y + prompt_h - 54, prompt_x + 158, prompt_y + prompt_h - 16};
    acw_draw_round_rect(dc, agent, ACW_COLOR_SURFACE, ACW_COLOR_BORDER, 18);
    acw_draw_text(dc, g_app.font_small, L"Agent", agent, ACW_COLOR_TEXT, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    RECT model = {prompt_x + prompt_w - 464, prompt_y + prompt_h - 54, prompt_x + prompt_w - 310, prompt_y + prompt_h - 16};
    acw_draw_text(dc, g_app.font_ui, L"K2.6 思考⌄", model, ACW_COLOR_TEXT, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);

    RECT ai = {prompt_x + prompt_w + 8, prompt_y + prompt_h - 60, prompt_x + prompt_w + 48, prompt_y + prompt_h - 20};
    acw_draw_round_rect(dc, ai, ACW_COLOR_ACCENT, ACW_COLOR_ACCENT, 14);
    acw_draw_text(dc, g_app.font_ui_bold, L"AI", ai, RGB(255, 255, 255), DT_CENTER | DT_VCENTER | DT_SINGLELINE);

    RECT tray_title = {card_x, template_y - 26, card_x + card_w, template_y - 6};
    acw_draw_text(dc, g_app.font_micro, L"常用办公技能", tray_title, ACW_COLOR_FAINT, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    ...
    acw_draw_round_rect(dc, file_panel, ACW_COLOR_PANEL, ACW_COLOR_BORDER, 16);
    acw_draw_round_rect(dc, artifact_panel, ACW_COLOR_PANEL, ACW_COLOR_BORDER, 16);
    ...
    acw_draw_text(dc, g_app.font_ui_bold, file_label, file_title, ACW_COLOR_TEXT, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
    RECT file_caption = {file_panel.left + 20, file_panel.top + 38, file_panel.right - 20, file_panel.top + 56};
    acw_draw_text(dc, g_app.font_micro, L"只读取已信任目录，默认禁止全盘扫描", file_caption, ACW_COLOR_FAINT, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);

    RECT artifact_title = {artifact_panel.left + 20, artifact_panel.top + 14, artifact_panel.right - 20, artifact_panel.top + 40};
    acw_draw_text(dc, g_app.font_ui_bold, L"计划 / 产物 / 审批", artifact_title, ACW_COLOR_TEXT, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
    RECT artifact_caption = {artifact_panel.left + 20, artifact_panel.top + 38, artifact_panel.right - 20, artifact_panel.top + 56};
    acw_draw_text(dc, g_app.font_micro, L"生成草稿、预览 diff、审批后才执行本地操作", artifact_caption, ACW_COLOR_FAINT, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);

    RECT status = {content_x + 28, client.bottom - 36, client.right - 24, client.bottom - 14};
    acw_draw_text(dc, g_app.font_small, g_app.status_line, status, ACW_COLOR_MUTED, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
}

static void acw_paint(HWND window) {
    PAINTSTRUCT paint;
    ...
    int sidebar = acw_min_int(300, acw_max_int(260, (client.right - client.left) / 5));
    acw_draw_sidebar(dc, client, sidebar);
    acw_draw_main(dc, client, sidebar);

    EndPaint(window, &paint);
}

static void acw_init_resources(void) {
    g_app.font_ui = acw_create_font(17, FW_NORMAL);
    g_app.font_ui_bold = acw_create_font(17, FW_SEMIBOLD);
    g_app.font_small = acw_create_font(14, FW_NORMAL);
    g_app.font_micro = acw_create_font(12, FW_NORMAL);
    g_app.font_heading = acw_create_font(20, FW_SEMIBOLD);
    g_app.font_brand = acw_create_named_font(68, FW_HEAVY, L"Arial Black");
    g_app.font_title = acw_create_font(24, FW_NORMAL);
    g_app.brush_bg = CreateSolidBrush(ACW_COLOR_BG);
    g_app.brush_surface = CreateSolidBrush(ACW_COLOR_SURFACE);
    g_app.brush_panel = CreateSolidBrush(ACW_COLOR_PANEL);
    g_app.selected_template = 0;
    acw_set_status(L"Office Mode 就绪。请选择一个本地工作区开始。");
}

static void acw_destroy_resources(void) {
...
static LRESULT CALLBACK acw_window_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam) {
    switch (message) {
    case WM_CREATE:
        acw_init_resources();
        kcw_native_bridge_init(window);
        acw_create_controls(window);
        acw_layout_controls(window);
        acw_detect_initial_workspace();
        if (g_initial_workspace[0] != L'\0') {
    ...
    case WM_CTLCOLORLISTBOX: {
        ...
        SetTextColor(dc, ACW_COLOR_TEXT);
        if (child == g_app.file_list || child == g_app.artifact_edit) {
            SetBkColor(dc, ACW_COLOR_PANEL);
            return (LRESULT)g_app.brush_panel;
        }
        if (child == g_app.prompt_edit) {
            SetTextColor(dc, ACW_COLOR_MUTED);
        }
        SetBkColor(dc, ACW_COLOR_SURFACE);
        return (LRESULT)g_app.brush_surface;
    }

    case WM_DRAWITEM:
        acw_draw_button((const DRAWITEMSTRUCT *)lparam);
        return TRUE;

    case WM_COMMAND: {
        int id = LOWORD(wparam);
        if (id == ACW_ID_BROWSE) {
            acw_select_workspace(window);
            return 0;
        }
        if (id == ACW_ID_RUN) {
            acw_generate_plan(window);
        ...
        if (id == ACW_ID_APPROVE) {
            acw_approve_plan(window);
            return 0;
        }
        if (id == ACW_ID_NEW_CHAT) {
            ...
            acw_set_status(L"新会话已创建。");
            InvalidateRect(window, NULL, TRUE);
            return 0;
        }
        if (id == ACW_ID_DEVELOPER) {
            SetWindowTextW(
                g_app.artifact_edit,
                L"Developer Mode\r\n\r\n"
            ...
            acw_set_status(L"Developer Mode 面板已打开。");
            InvalidateRect(window, NULL, TRUE);
            return 0;
        }
        if (id == ACW_ID_BROWSER) {
            ...
            if (g_app.webview_visible) {
                ...
                acw_set_status(L"浏览器视图已打开。");
            } else {
                acw_set_status(L"已返回原生视图。");
            }
            acw_layout_controls(window);
            InvalidateRect(window, NULL, TRUE);
            return 0;
        }
        if (acw_is_template_id(id)) {
            g_app.selected_template = id - ACW_ID_TEMPLATE_BASE;
            wchar_t status[256];
            swprintf_s(status, sizeof(status) / sizeof(status[0]), L"已选择模板：%s。", ACW_TEMPLATES[g_app.selected_template]);
            acw_set_status(status);
            InvalidateRect(window, NULL, TRUE);
            return 0;
        }
...
int kcw_run_app_with_workspace(HINSTANCE instance, int show_command, const wchar_t *initial_workspace) {
    ...
    window_class.lpfnWndProc = acw_window_proc;
    window_class.hInstance = instance;
    window_class.lpszClassName = ACW_CLASS_NAME;
