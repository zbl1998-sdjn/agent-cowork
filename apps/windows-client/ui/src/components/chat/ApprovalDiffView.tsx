...
// 二进制内容只报字节数；改动过大跳过逐行 diff,回退到整块 before/after。纯展示。
...
function BinaryPreview({ preview }: { preview: BinaryDiffPreview }) {
  ...
  return (
    <div className="approval-diff approval-diff--binary">
      <div className="approval-diff-path">{preview.path}</div>
...
function TextPreview({ preview }: { preview: TextDiffPreview }) {
  ...
  return (
    <div className="approval-diff">
      <div className="approval-diff-path">{isNewFile && <span className="approval-diff-new-badge">新建文件</span>}{preview.path}</div>
      {lines === null ? (
        <div className="approval-diff-toobig">
          <p>改动过大，不逐行显示；以下分别是修改前/修改后：</p>
          {preview.before != null && <pre className="approval-diff-block">{preview.before}</pre>}
          <pre className="approval-diff-block">{preview.after}</pre>
        </div>
      ) : (
