(function () {
    function renderRunEventPayload(type, payload, appendLine) {
        ...
        if (type === "assistant_end") {
            appendLine(payload.status === "failed" ? "failed" : "done", payload.status === "failed" ? "运行失败" : "运行完成", payload.durationMs != null ? `${payload.durationMs}ms` : "");
            return true;
        }
        return false;
    }
    ...
    function subscribeRunEvents(message, runId, options = {}) {
        ...
        (async () => {
            try {
                ...
                if (!closed) {
                    const error = err;
                    if (error && error.name !== "AbortError" && typeof options.onError === "function")
                        options.onError();
                }
            }
            finally {
