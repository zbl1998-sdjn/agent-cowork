...
const composer = requiredElement(".composer textarea");
const composerPopover = requiredElement(".composer-popover");
const uploadInput = requiredElement(".upload-input");
const folderInput = requiredElement(".folder-input");
const approveButton = requiredElement(".approve-button");
const sendButton = requiredElement(".send-button");
const artifactText = requiredElement(".artifact-preview p");
const artifactPath = requiredElement(".artifact-preview code");
const statusText = requiredElement(".status-text");
const runChip = requiredElement(".run-chip");
const runSummary = requiredElement(".run-summary");
const runList = requiredElement(".run-list");
const runRefreshButton = requiredElement('[data-action="refresh-runs"]');
const artifactList = requiredElement("[data-artifact-list]");
const artifactRefreshButton = requiredElement('[data-action="refresh-artifacts"]');
const workspacePath = requiredElement(".workspace-card > strong");
const workspaceMeta = requiredElement(".workspace-card > p");
const fileList = requiredElement(".file-list");
const operationList = requiredElement(".operation-list");
const chatOutput = requiredElement(".chat-output");
const chatOutputText = requiredElement(".chat-output p");
const conversationTimeline = requiredElement(".conversation-timeline");
const conversationEmpty = requiredElement(".conversation-empty");
const workbenchTitle = requiredElement(".workbench-title");
const workbenchCopy = requiredElement(".workbench-copy");
const interactionSubtitle = requiredElement(".interaction-subtitle");
const interactionItems = requiredElement(".interaction-items");
const recipeSummary = requiredElement(".recipe-summary");
const recipeList = requiredElement(".recipe-list");
const recipeClearButton = requiredElement('[data-action="clear-recipe"]');
const clarifyPanel = requiredElement(".clarify-panel");
const clarifyOptions = requiredElement(".clarify-options");
const placeholders = {
    chat: "今天想让 Kimi 做什么？",
...
const { arrayBufferToBase64, basename, compactText, formatDuration, formatRunTime, idempotencyKey, joinWin, messageStatusClass, runStatusText, runTypeText, shortRunId, uniqueStamp, } = window.AgentCoworkUtils;
...
const { setStatus, setRunChip, renderInteraction, resetInteraction, setView, summarizeFiles, renderFiles, renderOperations, selectedRecipe, renderRecipes, } = window.AgentCoworkWorkbenchRenderer.createWorkbenchRenderer({
    state,
    composer,
    placeholders,
    statusText,
...
const { textCandidate, activeFiles, recipeFiles, maybeSelectRecipe, shouldClarify, shouldUseCowork, readCandidateSummary, } = window.AgentCoworkTaskContext.createTaskContext({
    state,
    postJson,
    basename,
    selectedRecipe,
...
const { showChatResponse, syncConversationState, scrollConversationToEnd, setMessageStatus, appendUserMessage, appendAssistantMessage, appendMessageText, addProgressLines, addPreviewCard, markApprovalDone, addArtifactCard, addSourcesFooter, clearConversation, } = window.AgentCoworkMessageRenderer.createMessageRenderer({
    state,
    composer,
    chatOutput,
    chatOutputText,
...
async function generatePlan(options = {}) {
    ...
    setArtifact(kimiPlan.used
        ? `已读取本地内容：${summary}；Kimi API 已生成计划，运行记录 ${shortRunId(kimiPlan.runId)}。`
        : `已读取本地内容：${summary}`, outputPath.replace(state.workspace, "."));
    if (kimiPlan.used) {
