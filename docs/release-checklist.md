...
- [ ] Q8/R5: Windows installer or installed-client smoke status is recorded as JSON under `reports/windows-client-smoke/` with `npm run smoke:windows-client -- -DryRun` at minimum, and with the installed executable path when available.
...
npm run smoke:e2e
npm run bench
npm run smoke:windows-client -- -DryRun
...
npm run smoke:windows-client -- -InstallerPath <path-to-installer> -InstalledExePath <path-to-installed-AgentCowork.exe>
