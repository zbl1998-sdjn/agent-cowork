...
打开 **Agent Cowork**,启动时会先拉起本机后台服务(host),随后进入登录界面:
...
> 账户与会话存储在本机 SQLite(`<用户主目录>\.AgentCowork\auth.sqlite`),**重启后不会丢失**。
...
在打包好的桌面应用里也跑通了**门禁开启**下的完整链路:启动 → 登录(游客)→ 新建对话 → 发送消息 → 收到 `kimi-k2.6` 真实流式回复(即第五节那张截图)。命令面板、工具、连接器、可视化、设置等面板均逐一验证正常。

下图是**安装版**(从 NSIS 安装、正常双击启动)的实拍:启动后内置 host 自动拉起、跳过闪屏、自动登录并给出真实回复:

![安装版桌面应用真实运行](images/installed-login.png)

排查中定位并修复了两个会影响"能否正常使用"的真实缺陷:
...
- 本机服务默认要求登录令牌才能调用接口,且不信任伪造的身份头;CSP 锁定 `object-src 'none'`,CORS 只放行本机回环与桌面 WebView(`tauri.localhost`),拒绝 `Origin: null`。
