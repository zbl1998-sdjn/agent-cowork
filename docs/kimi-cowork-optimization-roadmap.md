...
- `.AgentCowork/runs/*.json` 已经有数据, 缺一个常驻面板把它渲染成有状态的任务列表。
...
- Go Local Agent 当前只跑文件 op; 新增 `tool exec` 子命令: 接受 `{ tool, input }` JSON, 调一个 sidecar Python (venv 在 `~/.agent-cowork/py/`) 执行, 全程 stdin/stdout JSON, 不暴露 shell。
- 注册四类 tool:
...
- 每次模板跑完, 除了写 .xlsx, 同时生成 `.AgentCowork/artifacts/<runId>.html` — 自包含 HTML, 引一个 `kimi-artifact.js`。
- 这个 JS 提供两个原语:
...
- 项目级 `CLAUDE.md` 等价物: `<trusted_root>/.AgentCowork/MEMORY.md` + `memory/*.md`。
