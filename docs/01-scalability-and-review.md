...
- **P5 抗泄漏/浸泡 + 实测基准** ✅（可跑部分）：`concurrency-soak.test.js`(40 路并发集体断开 → 注册表归零)；`scripts/bench-local.mjs` 单实例实测——150 并发 920 流/s p99≈147ms、500 并发 1064 流/s p99≈432ms,**0 错误且 approvals/runs/slots 全部归零(无泄漏)**;`apps/host/scripts/load-sse.ts` 多实例集群压测脚手架。**1万→10万** 需多实例 + 负载均衡(由 P2 跨实例机制支撑),按 500/实例 线性外推。
