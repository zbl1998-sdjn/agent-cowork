...
> 本文档面向本仓库的开发者:完整的本地门禁、烟测与评测命令、MASE 长期记忆桥接配置,以及冷冻预研代码的历史自检方式。产品介绍见根目录 [README](../README.md)。
...
```powershell
Add-MpPreference -AttackSurfaceReductionOnlyExclusions "<仓库根目录>\build\windows-client-vs\AgentCowork.exe"
...
```text
同意为 <仓库根目录>\build\windows-client-vs\AgentCowork.exe 添加 Microsoft Defender ASR-only 精确路径排除项
```
