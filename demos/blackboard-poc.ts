...
type Fact = { value: string; agent: string; ts: string };

const sleep = (ms: number): Promise<void> => new Promise((resolve) => setTimeout(resolve, ms));
const now = () => new Date().toISOString().replace('T', ' ').slice(0, 19);

// ---------- 确定性写入闸门(单写者;此处没有任何 LLM)----------
const facts = new Map<string, Fact>();   // entity -> { value, agent, ts }
const audit: string[] = [];          // 审计链
function writeFact(agent: string, entity: string, value: string, source: string): void {
...
function logRead(agent: string, note: string): void { audit.push(`[${now()}] ${agent} 读取 → ${note}`); render(); }
...
(async () => {
  ...
  let shareOK: boolean | null = null, govOK: boolean | null = null;

  // Agent A(recorder)— 提议事实 → 闸门落盘到黑板
  console.log(Y + '\n[Agent A / recorder] 写入项目事实 → 黑板' + Z);
  if (token) await ask(token, conv, 'You are Agent A (recorder). Record these project facts: deploy port is 8080, database is PostgreSQL. Reply only "recorded".');
  ...
  if (token) {
    const b = await ask(token, conv, 'You are Agent B (config generator). Using the project deploy port and database that were ALREADY decided earlier, output ONE startup config line using those exact values.');
  ...
  if (token) await ask(token, conv, 'You are Agent C. Correction: the deploy port is now 9090, not 8080. Reply only "updated".');
  ...
  if (token) {
    const d = await ask(token, conv, 'What is the CURRENT deploy port for this project? Answer the number only.');
