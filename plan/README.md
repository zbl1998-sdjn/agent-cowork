...
- [x] P0-T10a/Q9(本地可测子项):已新增 host opt-in `checkJs` + JSDoc 类型护栏,覆盖 97 个 host 模块(新增 file-preview、live artifact spec/viz、HTTP request utils、SQLite storage、OAuth GitHub/device flow、approval/OAuth/file-operation permission stores、JSON store、tool registry、Office writer、model recorder、audit/jsonl writer、clarification/background task store、run-store/index facade/file/sqlite/utils、scheduler/store、subagent runner/parallel、sandbox execution stack、hooks、agent-resume、provider router、plan builder、runtime dependencies status/plan/detectors、run event/trace stack、data table/statistics parser 等边界),并加入 host Node 内建最小类型声明;已接入 `npm run check`。
- [ ] P0-T10 延后项:完整 Node 类型覆盖、更多 L0/L1 模块纳入仍在推进;迁移前架构门禁已覆盖 host `.ts` source 并支持 `.js` specifier 回落同名 `.ts`,但逐文件 `.ts` 转换尚未开始,仍需先明确直接 Node 运行下的导入/打包路径;不改 Node host 语言栈、不重写。
...
- [x] 06-F5b 更新保留计划预检 UI:运行时依赖面板可向 `/api/runtime/dependencies/update-plan` 生成更新保留预案,展示 AppData 根、按需组件/venv/用户数据/缓存保留项、installer invariant 和破坏性动作数量;仅展示可审查计划,不执行安装、下载或删除。UI/API/view-model 聚焦测试通过。
...
- [x] 05-A4a CSV/TSV 数据分析闭环(本地可测子项):新增只读 `data.analyze` builtin 与 agent `AnalyzeDataFile` 增强,可对 trustedRoot 内 CSV/TSV 输出列统计、聚合图表 spec 和 Markdown 报告草稿;不写 artifact、不安装重依赖、不联网。新增 host 单测覆盖图表/报告/builtin/agent,并纳入 host `checkJs`。Excel 读取、真实图表 artifact 与 pandas/numpy/matplotlib 增强仍留后续切片。
...
- [x] 06-D3/B1-B6 按需下载供应链预检(本地可测子项):按需组件 catalog 已暴露 `sourceKind/sourceUrl/sha256/signaturePolicy`;安装计划会对任何缺来源 URL、缺有效 sha256 或缺签名策略的下载组件 `ok:false` fail-closed,仅返回可审查计划,不触网、不执行安装器、不写 PATH/注册表。聚焦 `runtime-dependencies.test.js` 通过。
...
- [x] 07-D2d 取消后 checkpoint 续跑验收:手动取消导致的模型 abort 会按 `cancelled` 安全收尾,不会误走 error 帧;HTTP/SSE E2E 覆盖写入后取消再用同一 `resumeRunId` 续跑,确认模型看到已完成 tool result 且 `Write` 副作用不重放。
...
- [x] P2-B2 smoke 无配置补证:`smoke:react-connectors` 已在 UI 闭环前加入 GitHub OAuth no-client-id 预检,确认 status 未配置、start 428、不触外网、不消耗 approval、不落 token,并在 env client id 恢复后复用同一 approval 且忽略 body `clientId`;结果写入脱敏报告 `missingGitHubClientId`。
...
- [x] P3-B2 provider router 真实接入:`kimi/provider/router.js` 已用于真实 agent 模型 fallback 调度,支持配置对象候选和 `shouldFallback` 谓词,按 `provider|baseUrl|model` 保持同 provider 不同 endpoint/model 的 breaker 粒度;agent stream E2E 覆盖 primary 失败后 fallback 继续写文件,且 SSE/run record 不泄漏 primary/fallback key。
...
- [x] FE-2a 局部迁移(ToolsPanel):工具面板与状态视图/结果分类单测迁入 `components/panels/`,同步懒加载入口和 `panel-tools` 分包规则;工具搜索、调用、计划追加和需审批工具禁用行为不变。
...
- [x] Host 体积治理(recipes registry):`recipes/registry.js` 拆出 `recipe-helpers.js` 的 source/artifact/operation/table/action helper,保留 recipe id/output/riskLevel、artifact path/encoding 和审批 operation shape;聚焦 recipe/server 测试与门禁通过,体积软警告降为 9 个。
