...
- [x] P0-T10a/Q9(本地可测子项):已新增 host opt-in `checkJs` + JSDoc 类型护栏,首批覆盖 4 个纯模块并接入 `npm run check`。
- [ ] P0-T10 延后项:Node 内建类型覆盖、更多 L0/L1 模块纳入与逐文件 `.ts` 转换尚未开始;不改 Node host 语言栈、不重写。
- [x] P1-A1/P1-A2:后端 `todo_snapshot/todo_update` 事件 + 前端执行清单组件已接入;host/UI 单测与 `npm run ci` 通过。
- [ ] P1-A3:真实多文件任务的"研究→计划→批准→执行→自检→收尾"端到端留档未完成。
...
- [ ] 继续:按未完成清单推进 P0-T10/Q9 host 类型护栏、P1-A3、FE 真实滚动验证、03-B1 定时/连接器/浏览器验收尾项、05-B1b 体验尾项、05-A2 Office 深操作等
