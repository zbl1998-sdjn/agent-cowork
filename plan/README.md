...
- [x] P1-D1(代码+单测完成):新增 approval exact-ID 批量 resolve 能力:in-memory registry、Postgres approval store 与 `/api/approvals/batch` 均支持显式 id 列表批量审批,逐个保留 tenant/user scope 校验和 decision 归一化;不支持 wildcard/runId/all,不做同类工具通配自动批准。新增 host 单测覆盖非法 id、跨租户拒绝、PG 跨实例与异步 store 路由。
