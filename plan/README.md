...
- [x] P0-T10a/Q9(本地可测子项):已新增 host opt-in `checkJs` + JSDoc 类型护栏,覆盖 26 个纯/叶子/安全链路模块(新增 file-preview、live artifact spec/viz、OAuth GitHub/device flow、OAuth permissions、JSON store、tool registry、Office writer 等边界),并加入 host Node 内建最小类型声明;已接入 `npm run check`。
