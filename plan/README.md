...
| `08-企业机密档安全基线.md` | 机密档威胁模型、三档承诺(标准/内网/隔离)、机密模式/落盘加密/OS 级出网强制切片 | 企业保密能力任务 |
...
- [x] 03-B5 起始建议/后续 chips:空会话 starter 已按最近 run 与 recipe 摘要生成上下文建议,再回落默认建议;assistant `suggestions` 围栏 chips 继续一键发送。新增 UI 纯函数单测并复用 Timeline chips 渲染测试,不新增路由或文件写入面。
...
- [x] 05-A4a CSV/TSV/XLSX 数据分析闭环(本地可测子项):只读 `data.analyze` builtin 与 agent `AnalyzeDataFile` 可对 trustedRoot 内 CSV/TSV 和 XLSX 首个 worksheet 输出列统计、聚合图表 spec 和 Markdown 报告草稿;新增审批型 `data.createChartArtifact`/`CreateDataChartArtifact` 可把推荐图表保存为 `.AgentCowork/artifacts` 活页 artifact。图表写入显式 `mutating/high/requiresApproval`,不执行公式、不安装重依赖、不联网。host 单测覆盖图表/报告/builtin/agent、XLSX 读取、artifact manifest 和审批型工具描述符,并纳入 host `checkJs`。pandas/numpy/matplotlib 增强仍留后续切片。
...
- [x] 06-F9 离线可用性:新增 `npm run smoke:offline-local`(`scripts/smoke-offline-local.ts`),在清空 Kimi/proxy 环境并拦截非本机 fetch 的情况下验证 health/workspace/文件读取/文件写入/依赖状态/audit 仍可用;模型路由无 key 时返回中文说明"本地文件功能仍可离线使用,模型回复需联网配置 key",证据见 `reports/offline-local/offline-local-smoke-2026-05-25T18-09-33-602Z.json`。
