...
- [x] 05-A4a CSV/TSV/XLSX 数据分析闭环(本地可测子项):只读 `data.analyze` builtin 与 agent `AnalyzeDataFile` 可对 trustedRoot 内 CSV/TSV 和 XLSX 首个 worksheet 输出列统计、聚合图表 spec 和 Markdown 报告草稿;不写 artifact、不执行公式、不安装重依赖、不联网。host 单测覆盖图表/报告/builtin/agent 和 XLSX 读取,并纳入 host `checkJs`。真实图表 artifact 与 pandas/numpy/matplotlib 增强仍留后续切片。
