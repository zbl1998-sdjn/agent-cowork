...
D2d 完成记录(2026-05-26):手动取消触发的模型 abort 现在按 `cancelled` 安全收尾,不会误走 error 帧;已补 HTTP/SSE E2E 覆盖"写入文件后取消→同一 `resumeRunId` 从 checkpoint 续跑→已完成 `Write` 不重放",确认续跑模型可看到已完成 tool result,文件副作用仍只出现一次。
